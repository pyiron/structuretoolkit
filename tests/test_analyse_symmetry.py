...
class TestSymmetry(unittest.TestCase):
    ...
    def test_get_primitive_cell_functional(self):
        cell = 2.2 * np.identity(3)
        basis = Atoms(
            "AlFe", scaled_positions=[(0, 0, 0), (0.5, 0.5, 0.5)], cell=cell, pbc=True
        )
    ...
    def test_get_primitive_cell_hex_functional(self):
        elements = ["Fe", "Fe", "Fe", "Fe", "O", "O", "O", "O", "O", "O"]
        positions = [
            [0.0, 0.0, 4.89],
            [0.0, 0.0, 11.78],
    ...
    def test_get_equivalent_points(self):
        basis = Atoms(
            "FeFe",
            positions=[[0.01, 0, 0], [0.5, 0.5, 0.5]],
            cell=np.identity(3),
            pbc=True,
        )
    ...
    def test_get_equivalent_points_functional(self):
        basis = Atoms(
            "FeFe",
            positions=[[0.01, 0, 0], [0.5, 0.5, 0.5]],
            cell=np.identity(3),
    ...
    def test_get_space_group(self):
        ...
        Al_fcc = Atoms("Al", scaled_positions=[(0, 0, 0)], cell=cell, pbc=True)
        self.assertEqual(
        ...
        Mg_hcp = Atoms("Mg2", scaled_positions=pos, cell=cell, pbc=True)
        self.assertEqual(
        ...
        Mg_hcp = Atoms("Mg4", scaled_positions=pos, cell=cell, pbc=True)
        self.assertEqual(
            stk.analyse.get_symmetry(structure=Mg_hcp).spacegroup["Number"], 194
        )
