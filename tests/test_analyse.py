...
class TestAtoms(unittest.TestCase):
    @unittest.skipIf(
        skip_cluster_test, "sklearn is not installed, so the cluster tests are skipped."
    )
    def test_get_layers(self):
        ...
        self.assertAlmostEqual(
            np.linalg.norm(layers - np.rint(2 * struct.positions / a_0).astype(int)), 0
        )
        ...
        self.assertTrue(
            np.all(
                stk.analyse.get_layers(structure=struct)
                == stk.analyse.get_layers(
                    structure=struct,
                    cluster_method=AgglomerativeClustering(
                        linkage="complete", n_clusters=None, distance_threshold=0.01
                    ),
                )
            ),
        ...
        self.assertTrue(
            np.all(
                stk.analyse.get_layers(structure=struct_pure)
                == stk.analyse.get_layers(
                    structure=struct_pure, cluster_method=DBSCAN(eps=0.01)
                )
            ),
    ...
    def test_get_layers_with_strain(self):
        ...
        stk.common.apply_strain(
            structure=structure, epsilon=0.1 * (np.random.random((3, 3)) - 0.5)
        )
        self.assertEqual(
            layers,
            stk.analyse.get_layers(
                structure=structure, planes=np.linalg.inv(structure.cell).T
            ).tolist(),
        )
    ...
    def test_get_layers_across_pbc(self):
        ...
        self.assertEqual(
            len(
                np.unique(
                    layers[stk.analyse.get_layers(structure=structure)[:, 0] == 0, 0]
                )
            ),
            1,
        )
            ...
            all(
                [
                    ...
                    for v in stk.analyse.get_centro_symmetry_descriptors(
                        structure=basis, num_neighbors=8
                    )
                ]
            )
        )
    ...
    def test_get_voronoi_vertices(self):
        ...
        self.assertEqual(
            len(
                stk.analyse.get_voronoi_vertices(structure=basis, distance_threshold=2)
            ),
            1,
        )
    ...
    def test_get_interstitials_bcc(self):
        ...
        x_octa_ref = stk.common.get_wrapped_coordinates(
            structure=bcc, positions=x_octa_ref
        )
        ...
        x_tetra_ref = stk.common.get_wrapped_coordinates(
            structure=bcc, positions=stk.analyse.get_voronoi_vertices(structure=bcc)
        )
    ...
    def test_get_interstitials_fcc(self):
        ...
        a_0 = fcc.cell[0, 0]
        x_tetra_ref = (
            0.25 * a_0 * np.ones(3) * np.array([[1], [-1]]) + fcc.positions[:, None, :]
        )
        x_tetra_ref = stk.common.get_wrapped_coordinates(
            structure=fcc, positions=x_tetra_ref
        ).reshape(-1, 3)
        ...
        x_octa_ref = stk.common.get_wrapped_coordinates(
            structure=fcc, positions=x_octa_ref
        )
        ...
        self.assertAlmostEqual(
            int_octa.get_variances().sum(), 0, msg="Distance variance in FCC must be 0"
        )
    ...
    def test_interstitials_details(self):
        ...
        self.assertGreater(
            tree.query(x)[0].min(),
            1,
            msg="remove_too_close did not remove closest points",
        )
        self.assertGreater(
            len(x),
    ...
    def test_strain(self):
        ...
        center = voro.vertices[
            np.linalg.norm(
                voro.vertices - structure.cell.diagonal()[:2] * 0.5, axis=-1
            ).argmin()
        ]
        structure.positions[:, 2] += (
            b / (2 * np.pi) * np.arctan2(*(structure.positions[:, :2] - center).T[::-1])
        )
        structure = stk.common.center_coordinates_in_unit_cell(structure=structure)
        r_0 = 0.9 * L / 2
        ...
        strain = stk.analyse.get_strain(
            structure=structure, ref_structure=structure_bulk, num_neighbors=8
        )
    ...
    def test_tessellations(self):
        ...
        self.assertAlmostEqual(
            np.linalg.norm(
                stk.analyse.find_mic(
                    structure=structure,
                    v=np.diff(
                        structure.positions[
                            stk.analyse.get_delaunay_neighbors(structure=structure)
                        ],
                        axis=-2,
                    ),
                ),
        ...
        self.assertAlmostEqual(
            np.linalg.norm(
                stk.analyse.find_mic(
                    ...
                    v=np.diff(
                        structure.positions[
                            stk.analyse.get_voronoi_neighbors(structure=structure)
                        ],
                        axis=-2,
                    ),
                ),
    ...
    def test_cluster_positions(self):
        structure_bulk = bulk("Fe", cubic=True)
        self.assertEqual(
            len(stk.analyse.get_cluster_positions(structure=structure_bulk)),
            len(structure_bulk),
        )
        positions = np.append(
            structure_bulk.positions, structure_bulk.positions, axis=0
        )
        self.assertEqual(
            len(
                stk.analyse.get_cluster_positions(
                    structure=structure_bulk, positions=positions
                )
            ),
            len(structure_bulk),
        )
        self.assertEqual(
            stk.analyse.get_cluster_positions(
                structure=structure_bulk, positions=np.zeros((2, 3)), return_labels=True
            )[1].tolist(),
            [0, 0],
        )
