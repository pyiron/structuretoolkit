...
class TestCompound(unittest.TestCase):
    ...
    def test_C15(self):
        ...
        self.assertEqual(
            structure.get_chemical_formula(), "Cu16Mg8", "Wrong chemical formula."
        )

        a_type_nn_distance = stk.analyse.get_neighbors(
            structure=bulk(a_type), num_neighbors=1
        ).distances[0, 0]
        self.assertAlmostEqual(
        ...
        self.assertLess(
            1,
            csa[0],
            msg="A site for AB_2 C15 should be significantly non-centro-symmetric.",
        )
        self.assertAlmostEqual(
            0, csa[1], msg="B site for AB_2 C15 should be nearly centro-symmetric."
        )

        num_a_neighs = 16
        num_b_neighs = 12
        neigh = stk.analyse.get_neighbors(
            structure=structure, num_neighbors=num_a_neighs
        )
    ...
    def test_D03(self):
        ...
        self.assertTrue(
            np.all(symbols[a_neighbors] == element_b),
            msg="A-type should only have B-type neighbors.",
        )

        b_neighbors = neigh.indices[symbols == element_b]
        sorted_vals, counts = np.unique(
            np.mean(symbols[b_neighbors] == "Al", axis=1), return_counts=True
        )
        self.assertAlmostEqual(
            0, sorted_vals[0], msg="Shared sub-lattice has no A-type neighbors."
        )
        self.assertAlmostEqual(
            0.5, sorted_vals[1], msg="B-type sub-lattice has half A-type neighbors."
        )
        self.assertEqual(
            4, counts[0], msg="Shared sub-lattice should be only 1/4 of atoms."
        )
